...
void write_output(DATA_TYPE* output)
{
	int i, j;
	FILE * fp;
...
__global__ void mean_kernel(DATA_TYPE *mean, DATA_TYPE *data)
{
	...
	if ((j >= 1) && (j < (M+1)))
	{
		...
		for(i = 1; i < (N+1); i++)
		{
			mean[j] += data[i * (M+1) + j];/*2*/
		}
		mean[j] /= (DATA_TYPE)FLOAT_N;/*3*/
	}
}
...
__global__ void covar_kernel(DATA_TYPE *symmat, DATA_TYPE *data)
{
	...
	if ((j1 >= 1) && (j1 < (M+1)))
	{
		...
		{		
			...
			for(i = 1; i < (N+1); i++)
			{
				symmat[j1 * (M+1) + j2] += data[i *(M+1) + j1] * data[i *(M+1) + j2];/*5*/
			}
			symmat[j2 * (M+1) + j1] = symmat[j1 * (M+1) + j2];
		}
	}/*6*/
...
int main()
{
	...
	write_output(symmat_outputFromGpu);
